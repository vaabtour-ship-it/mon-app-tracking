...
import { trackingData, trackingTranslations } from './data/mockdata';
import './App.css';
...
function TrackingResult() {
  ...
  // Récupération de l'ID (avec fallback sur le localStorage au cas où)
  const trackingNumber = suiviId || localStorage.getItem('trackingNumber');
  const currentTracking = trackingData[trackingNumber];

  // Si le numéro n'existe pas dans mockdata.js, gestion d'erreur propre
  if (!currentTracking) {
    return (
  ...
  // Récupération des bonnes traductions associées à la marque chargée
  ...
  // --- CONFIGURATION DYNAMIQUE DU DESIGN (Cyberpunk vs Denim) ---
  const styles = {
    btnBack: isSpider ? {
    ...
    card: isSpider ? {
      ...
      background: 'var(--card-bg, #fcfbfa)', borderTop: '5px solid #1a2a40', borderRadius: '12px',
      padding: '35px', color: 'var(--text-color, #1a1a1a)', boxShadow: '0 10px 30px rgba(0,0,0,0.1)'
    },
    titleBrand: isSpider ? {
  ...
  // Calcul du style des cercles d'étape (remplis ou vides selon l'avancement)
        ...
        <div className="tracking-card" style={styles.card}>
          ...
          <div style={{ marginBottom: isSpider ? '35px' : '30px', textAlign: 'center' }}>
            ...
            {isSpider ? (
              ...
              <h3 style={{ fontSize: '24px', fontWeight: '500', margin: '0', color: '#1a2a40' }}>{t.title}</h3>
            )}
          </div>
          ...
          {/* Boîte de statut textuel finale */}
